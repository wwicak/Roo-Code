...
export class ClineAstIntegration {
	...
	public updateConfig(config: Partial<ClineAstIntegrationConfig>): void {
		...
		if (config.enableAstCaching !== undefined) {
			...
			this.astProvider.setCacheEnabled(config.enableAstCaching)
		}
	 ...
	 * @param filePath Relative path to the file
	 */
	public async rollbackChange(filePath: string): Promise<{
		success: boolean
		message: string
	}> {
		...
		if (result) {
			...
			return {
				success: false,
				message: `Failed to roll back changes to ${filePath}. No backup available.`,
			}
		}
	}
	...
	public getBackupInfo(filePath: string): {
		...
		const backups = this.rollbackManager.getBackupInfo(filePath)

		return {
			hasBackups: backups.length > 0,
			count: backups.length,
			operations: backups.map((b) => b.operation),
		}
	}
